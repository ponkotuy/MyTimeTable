...
trait Logs {
    ...
    def info(ex:Throwable) = if (logger.isEnabledFor(ERROR)) logger.info(ex.toString,ex)
    ...
    def warn(ex:Throwable) = if (logger.isEnabledFor(ERROR)) logger.warn(ex.toString,ex)
